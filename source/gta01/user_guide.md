...
所有型号的 MIC 板共享相同的功能架构，均包含以下三个核心模块：

| 模块 | 丝印标识 | 功能 |
|:----:|:-------:|:-----|
| **接口 A（上行）** | JP2| 连接主板或上一级 MIC 板。单级使用时只接此口。 |
| **接口 B（下行）** | JP1| 连接下一级 MIC 板，仅级联时使用。 |
| **MIC × 4** |  | 底进音 MEMS 麦克风，环形板呈圆形分布，线形板沿长边线性排列。 |

**极性标识**：每个接口旁有**圆点标记**，圆点所在侧即为"+"方向。接线时必须确保两端"+"对"+"。
...
| 需求场景 | 推荐型号 | 理由 |
|:---------|:--------:|:-----|
| 360° 全向拾音 | BOD0038 | 环形阵列，全方位声源定位 |
| 长条形产品、空间充裕 | BOD0043 | 4 cm 间距，低频响应更优 |
| 长条形产品、窄边框 | BOD0044 | 板宽仅 17 mm，适配窄边 |
| 紧凑型小尺寸产品 | BOD0046 | 板长仅 115 mm |
